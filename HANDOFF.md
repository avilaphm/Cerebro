...
- Invited PAR-Q completion now emails Pedro at `COACH_NOTIFY_EMAIL`, `PEDRO_EMAIL`, or the existing `pedro@cerebroai.au` fallback after the intake note, PDF, client update, and invite completion have saved. The notification includes the client, assessment time, medical flag state, optional client note, and a direct M & L link. Email failure is logged but never rolls back or blocks the client submission.
- Client creation now supports a PAR-Q-first movement assessment intake. Pedro can send a private 14-day PAR-Q link, mark the session as already booked, choose its date/time, and create the confirmed 50-minute appointment plus 5-minute buffer. The appointment syncs through the repaired Google Calendar integration. Pre-booked clients skip the public calendar after completing the form; unbooked clients continue to the existing slot picker. Both paths store the same `movement_assessment_intake` note format already read by the M & L screen. The Add client modal uses a high-specificity glass opt-out for a fully opaque white surface, gives its assessment heading normal block spacing, and locks all nested dashboard scroll containers while open.
